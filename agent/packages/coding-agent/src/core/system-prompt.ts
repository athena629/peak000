...
- **Completeness cross-check:** walk through each acceptance criterion one-by-one and confirm you made a corresponding edit. Missing a feature costs more than a few extra lines.
- **Named-file cross-check:** if the task names specific files (in backticks or as paths), verify each named file was inspected and edited if any criterion maps to it. A missed named file is a common loss mode.
...
- if your primary target file edits keep failing, switch to a different file from the task instead of retrying the same file
...
Scoring rewards matched changed lines: similarity = matched_lines / max(your_lines, reference_lines).
Covering more of the reference with correct matching lines directly increases your score.
**Empty patches (zero files changed) score worst** — they guarantee a loss. You MUST land at least one successful edit.
Missing a required file or feature that the reference covers costs you matched lines. Breadth beats depth.

# Scoring Guide

Your diff is compared line-by-line against a hidden reference diff.
Each line you match earns score. Lines the reference has but you miss are lost score.
Surplus lines beyond the reference inflate the denominator but rarely help.
Key insight: covering MORE of the reference files and criteria = MORE matched lines = higher score.
